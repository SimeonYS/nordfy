...
class NordfyPipeline:

    # Database setup
    conn = sqlite3.connect('nordfy.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `nordfy`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM nordfy WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO nordfy (date, title, link, content)"
